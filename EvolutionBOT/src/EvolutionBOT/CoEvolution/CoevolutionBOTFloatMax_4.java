...
import Utils.individuos.comparators.IndividuoComparatorVersus4FloatMax;

/**
 *
 * @author antares
 */
public class CoevolutionBOTFloatMax_4 extends SimpleGenetic {

    public CoevolutionBOTFloatMax_4(String[] args) {
        super(args);
    ...
    while (!population.isEmpty()) {
      individuo t1 = null,t2 = null,t3 = null,t4 = null;
      char text = '1';
      for (int i_h = 0; i_h < n_hebras; i_h++) {

        if (!population.isEmpty()) {      
          
          int t = random.nextInt(population.size());
          t1 = population.remove(t);
          
          t = random.nextInt(population.size());
          t2 = population.remove(t);
          
          t = random.nextInt(population.size());
          t3 = population.remove(t);
          
          t = random.nextInt(population.size());
          t4 = population.remove(t);
...
//                  + " [" + population.size() + "]");
          ...
          hebras.add(new t_fitness_versus4(t1,t2,t3,t4,"hebra_" + text + ".txt"));

                
      future_population.add(t1);
      future_population.add(t2);
      future_population.add(t3);
      future_population.add(t4);
  ...
  public void main() {
    ...
    //Ordenamos
    Collections.sort(population, new IndividuoComparatorVersus4FloatMax());
